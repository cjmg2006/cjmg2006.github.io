...
function(a) {
    ...
    var d = navigator.userAgent.toLowerCase(),
        e = d.indexOf("chrome") > -1 && (d.indexOf("windows") > -1 || d.indexOf("macintosh") > -1 || d.indexOf("linux") > -1) && d.indexOf("mobile") < 0 && d.indexOf("nexus") < 0,
        f = {
            tap_pixel_range: 5,
    ...
    a.isTouchCapable = function() {
        ...
        setup: function() {
            var b, d, e, g, h = this,
                i = a(h);
            i.on(f.startevent, function(a) {
                return a.which && 1 !== a.which ? !1 : i.data("lastTouch") ? void 0 : (i.data("doubletapped", !1), b = a.target, i.data("callee1", arguments.callee), g = a.originalEvent, e = {
                    position: {
                ...
                var g = (new Date).getTime(),
                    j = i.data("lastTouch") || g + 1,
                    k = g - j;
                if (window.clearTimeout(d), i.data("callee2", arguments.callee), k < f.doubletap_int && k > 0 && a.target == b && k > 100) {
                    i.data("doubletapped", !0), window.clearTimeout(f.tap_timer);
                    var l = {
                            position: {
                                x: f.touch_capable ? a.originalEvent.touches[0].screenX : a.screenX,
                                y: f.touch_capable ? a.originalEvent.touches[0].screenY : a.screenY
                            },
                            offset: {
                                x: f.touch_capable ? a.originalEvent.touches[0].pageX - a.originalEvent.touches[0].target.offsetLeft : a.offsetX,
                                y: f.touch_capable ? a.originalEvent.touches[0].pageY - a.originalEvent.touches[0].target.offsetTop : a.offsetY
                            },
                            time: (new Date).getTime(),
                            target: a.target
                        },
                        m = {
                            firstTap: e,
                            secondTap: l,
                            interval: l.time - e.time
                        };
                    c(h, "doubletap", a, m)
                } else i.data("lastTouch", g), d = window.setTimeout(function() {
        ...
        setup: function() {
            ...
            e.on(f.startevent, function(a) {
                ...
                if (diff_x = i.x - j, diff_y = i.y - k, h == a.target && g && (new Date).getTime() - b < f.taphold_threshold && (i.x == j && i.y == k || diff_x >= -f.tap_pixel_range && diff_x <= f.tap_pixel_range && diff_y >= -f.tap_pixel_range && diff_y <= f.tap_pixel_range)) {
    ...
    function(a) {
        ...
        function e(b) {
            ...
            b.beforeSend = function(a, d) {
                ...
                e.state = {
                    ...
                    fragment: b.fragment,
                    timeout: b.timeout
                }, (b.push || b.replace) && window.history.replaceState(e.state, o.title, o.url), document.activeElement.blur(), o.title && (document.title = o.title), h.html(o.contents);
                var p = h.find("input[autofocus], textarea[autofocus]").last()[0];
                if (p && document.activeElement !== p && p.focus(), r(o.scripts), "number" == typeof b.scrollTo && a(window).scrollTop(b.scrollTo), "" !== f) {
                    var s = l(o.url);
                    s.hash = f, e.state.url = s.href, window.history.replaceState(e.state, o.title, s.href);
                    var t = a(s.hash);
        ...
        function h(b) {
            ...
            if (c && c.container) {
                ...
                if (d.length) {
                    ...
                    h ? (d.trigger("pjax:start", [null, j]), c.title && (document.title = c.title), d.html(h), e.state = c, d.trigger("pjax:end", [null, j])) : e(j), d[0].offsetHeight
        ...
        mouseup: "mouseup",
        load: "load"
    }, Modernizr.touch && (events.click = "tap", events.mouseup = "touchend"), select = {
        document: $(document),
        html: $("html"),
        body: $("body"),
        htmlBody: $("html, body"),
        ...
        iePlaceholder: function() {
            Modernizr.input.placeholder || $("input, textarea").placeholder()
        },
        googleMap: function() {
            var a = !0;
            Modernizr.touch && (a = !1);
            var b = new google.maps.LatLng(39.3483428, -104.869894),
                c = {
                    center: b,
                    zoom: 14,
                ...
                d = new google.maps.Map(document.getElementById("google-map"), c);
            d.set("styles", [{
                featureType: "water",
                stylers: [{
                    color: "#5acde0"
    ...
    function() {
        ...
        bindings: function() {
            select.window.on("scroll", function() {
                scrolling.fixedHeaderHandler(), scrolling.fadeHeaderText()
            })
        },
        fadeHeaderText: function() {
            var a = select.window.scrollTop(),
                b = Math.round(100 * (1 - .006 * a)) / 100;
            b >= -1 && select.heading.css("opacity", b)
        },
        fixedHeaderHandler: function() {
            var a = select.window.scrollTop(),
                b = 75;
            a > b ? (select.headerFixed.addClass("scrolled"), select.nav.addClass("scrolled")) : (select.headerFixed.removeClass("scrolled"), select.nav.removeClass("scrolled"))
        },
        scrollTop: function() {
            select.htmlBody.animate({
                scrollTop: 0
            }, global.speed, global.ease)
        }
    }, ui = {
        bindings: function() {
            select.almiLogo.on(events.click, ui.almiGifHandler), select.almiGif.on(events.click, ui.almiGifHandler)
        },
        almiGifHandler: function() {
            select.almiGif.hasClass("is-visible") && select.html.hasClass("almi-is-visible") && select.almiGifWrapper.hasClass("is-visible") ? (select.almiGif.removeClass("is-visible"), select.html.removeClass("almi-is-visible"), setTimeout(function() {
                select.almiGifWrapper.removeClass("is-visible")
            }, 500)) : (select.almiGif.addClass("is-visible"), select.almiGifWrapper.addClass("is-visible"), select.html.addClass("almi-is-visible"))
        },
        almiGifHide: function() {
            select.almiGif.removeClass("is-visible"), setTimeout(function() {
                select.html.removeClass("almi-is-visible"), select.almiGifWrapper.removeClass("is-visible")
            }, 500)
        },
        almiGifSizeHandler: function() {
            var a = select.window.height();
            select.almiGif.height(a)
        }
    }, nav = {
        bindings: function() {
            select.btnMenu.on(events.click, function() {
                nav.menuHandler("open")
            }), select.btnMenuClose.on(events.click, function() {
                nav.menuHandler("close")
            })
        },
        menuHandler: function(a) {
            "open" === a ? select.nav.fadeIn(global.speed).removeClass("is-closed").addClass("is-open") : "close" === a && (ui.almiGifHide(), select.nav.delay(500).fadeOut(global.speed).removeClass("is-open").addClass("is-closed"))
        }
    }, page = {
        pageLoad: function(a, b) {
            scrolling.scrollTop(), select.pjaxContainer.delay(300).fadeOut(global.speed), select.header.addClass("hide"), setTimeout(function() {
                select.logoLoader.addClass("anim-pulse")
            }, global.speed);
            var c = select.nav.find("[data-section='" + a + "']");
            select.nav.find(".active").removeClass("active"), c.addClass("active"), void 0 !== b ? select.body.attr("class", b) : select.body.attr("class", a)
        },
        pageLoaded: function() {
            setTimeout(function() {
                nav.menuHandler("close"), select.dotLoader.removeClass("anim-pulse"), select.logoLoader.removeClass("anim-pulse")
            }, 400), setTimeout(function() {
                select.header.removeClass("hide")
            }, 800), select.pjaxContainer.delay(800).fadeIn(global.speed), touch.modifyAjaxLinks()
        },
        pageLoadFromHistory: function(a, b) {
            void 0 !== b ? select.body.attr("class", a + " " + b) : select.body.attr("class", a);
            var c = select.nav.find("[data-section='" + a + "']");
            select.nav.find(".active").removeClass("active"), c.addClass("active")
        },
        pageUpdatePageTitle: function(a) {
            document.title = a
        },
        popStateHandler: function() {
            var a = window.history.state.url,
                b = window.history.state.title,
                c = window.history.state.fragment;
            if (a) {
                var d = a.split("/"),
                    e = a.substring(a.lastIndexOf("/") + 1);
                d.indexOf("studio") >= 0 ? page.pageLoadFromHistory("studio") : d.indexOf("contact") >= 0 ? page.pageLoadFromHistory("contact") : d.indexOf("journal") >= 0 ? page.pageLoadFromHistory("journal") : d.indexOf("portfolio") && page.pageLoadFromHistory("portfolio", e), page.pageUpdatePageTitle(b), select.heading.html(c)
            }
        }
    }, pjax = {
        bindings: function() {
            select.document.on(events.click, ".fancy-url", function(a) {
                a.preventDefault(), a.stopPropagation();
                var b = $(this).attr("href"),
                    c = $(this).data("url"),
                    d = $(this).data("section"),
                    e = $(this).data("project");
                return (!$(this).hasClass("active") || $(this).hasClass("nav-link") && "portfolio" === d || $(this).hasClass("nav-link") && "journal" === d) && ($(this).hasClass("project-grid-link") || $(this).hasClass("project-link") ? page.pageLoad(d, e) : (page.pageLoad(d), $(this).find(select.dotLoader).addClass("anim-pulse")), $(this).hasClass("yh-logo-fixed") && (b = c), setTimeout(function() {
                    $.pjax(Modernizr.touch ? {
                        url: c,
                ...
                page.pageLoaded()
            }), $(document).on("pjax:success", function(a) {
                var b = $(a.target).children().find(".page-heading").html(),
                    c = $(a.target).children().find(".project-image");
                select.heading.html(b);
                var d = location.href.split(location.host)[1];
                ga("send", {
                    hitType: "pageview",
                    page: d,
                    title: window.history.state.title
                }), images.imageSrcHandler(c), window.history.state.fragment = b, window.history.replaceState(window.history.state, window.history.state.title, window.history.state.url)
            }), $(document).on("pjax:timeout", function(a) {
                a.preventDefault()
            })
        }
    }, images = {
        ...
        imageSrcHandler: function(a) {
            $(a).each(function() {
                var a = $(this),
                    b = a.data("src-desktop"),
                    c = a.data("src-tablet"),
                    d = a.data("src-mobile");
                (device.desktopLarge && a.hasClass("browser") || device.desktop && a.hasClass("full") || device.desktopLarge && a.hasClass("full")) && a.attr("src", b), (device.desktop && a.hasClass("browser") || device.tablet && a.hasClass("browser") || device.tablet && a.hasClass("tablet") || device.tablet && a.hasClass("full")) && a.attr("src", c), (device.mobile && a.hasClass("browser") || device.mobile && a.hasClass("tablet")) && a.attr("src", d)
            })
        }
    }, utility = {
        window: {
            checkWindowSize: function() {
                device.mobile ? select.html.addClass("mobile") : select.html.removeClass("mobile"), device.tablet ? select.html.addClass("tablet") : select.html.removeClass("tablet"), device.desktop ? select.html.addClass("desktop") : select.html.removeClass("desktop"), device.desktopLarge ? select.html.addClass("desktop-large") : select.html.removeClass("desktop-large"), device.desktopMax ? select.html.addClass("desktop-max") : select.html.removeClass("desktop-max")
            }
        },
        ...
        setContentHeight: function() {
            var a = select.window.height();
            select.contentWrapper.css("minHeight", a)
        },
        windowResizeHandler: function() {
            device.redefineObject(), utility.window.checkWindowSize(), utility.setContentHeight(), images.imageSrcHandler(select.projectImage)
        }
    }, touch = {
        bindings: function() {
        ...
        modifyAjaxLinks: function() {
            Modernizr.touch && select.fancyUrl.each(function() {
                $(this).removeAttr("href")
            })
        }
    }, init = {
        everything: function() {
            nav.bindings(), ui.bindings(), pjax.bindings(), touch.bindings(), images.bindings(), scrolling.bindings(), utility.checkBrowsers(), utility.window.checkWindowSize(), utility.setContentHeight(), utility.windowResizeHandler(), scrolling.fixedHeaderHandler()
        }
...
>>>>>>> 5143aa201cc23bb19af7a2dc1c69ec2b9cb5f558
